...
class Slug
{
		...
		/**
		 * @var $uri - string - slug/uri varaible
		 * @var $update - int - Are we checking insert or update?
		 *
		 */
		 function _check_duplicate($uri,$update=0 )
        {
            $CI =& get_instance();

			if ($update==1) {
				$CI->db->select($this->id)->where($this->field, $uri);
				if ($CI->db->count_all_results($this->table) > 1) {
					return FALSE; // Cannot remame slug to an existing slug
				} else {
					return TRUE; // Slug is the same
				}
			} else { // Check for insertion
                $CI->db->select($this->field)->where($this->field, $uri);
